    ...
    override suspend fun analyzeVoice(
        ...
        try {
            ...
            val prompt = if (context.startsWith("СКОРОМОВКА")) {
                ...
                """
Ти - професійний тренер з дикції. Проаналізуй запис СКОРОМОВКИ.
...
3. pace - темп (не занадто швидко/повільно для скоромовки)

ОБОВ'ЯЗКОВО надай відповідь ТІЛЬКИ у форматі JSON:
{
  "overallScore": 75,
  "clarity": 80,
  "pace": 70,
  ...
  "pauseQuality": 60,
  "fillerWordsCount": 0,
  "feedback": "Детальний фідбек українською: що почув, які звуки вимовлені добре/погано",
  "recommendations": ["Порада щодо цільових звуків 1", "Порада 2"],
  "strengths": ["Сильна сторона вимови 1", "Сильна сторона 2"]
}
                """.trimIndent()
            } else {
                // Стандартний промпт для інших вправ
                """
