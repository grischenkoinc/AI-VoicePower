...
class NegotiationViewModel @Inject constructor(
    ...
    fun onEvent(event: NegotiationEvent) {
        when (event) {
            ...
            NegotiationEvent.StartRecording -> {
                startRecording()
            }

            NegotiationEvent.StopRecording -> {
                stopRecording()
            }

            NegotiationEvent.NextStep -> {
                moveToNextStep()
            }
        }
    }
    ...
    private fun startRecording() {
        viewModelScope.launch {
            try {
                val outputFile = context.filesDir.resolve("recordings/negotiation_${UUID.randomUUID()}.m4a")
                ...
                Log.e("NegotiationVM", "Recording start error", e)
    ...
    private fun stopRecording() {
        viewModelScope.launch {
            try {
                ...
                Log.e("NegotiationVM", "Recording stop error", e)
                _state.update { it.copy(isRecording = false) }
                moveToNextStep()
            }
        }
    ...
    private suspend fun saveRecording(step: NegotiationStep?) {
        try {
            ...
            voiceAnalysisRepository.analyzeRecording(
                audioFilePath = path,
                expectedText = null,
                exerciseType = "negotiation",
                context = "Переговори, крок ${step?.stepNumber ?: 0}: ${step?.question ?: ""}"
            )
            ...
            val recordingEntity = RecordingEntity(
                ...
                type = "improvisation",
                contextId = "negotiation_step_${_state.value.currentStepIndex}",
                transcription = null,
                isAnalyzed = true,
            ...
            Log.e("NegotiationVM", "Save recording error", e)
        }
    }
